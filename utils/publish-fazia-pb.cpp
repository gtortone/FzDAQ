...
int main(int argc, char* argv[]) {
   ...
   //pub->bind("tcp://eth6:5563");
   //pub->bind("tcp://wlan0:5563");
   pub->bind("tcp://lo:5563");
...
while(1) {   
   ...
   EVSET.SerializeToString(&str);
   pub->send(str.data(), str.size());

   std::cout << "I: event size " << str.size() << std::endl;
